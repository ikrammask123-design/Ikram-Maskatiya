  ...
  const handleCheckoutGoogleAuth = async () => {
    ...
    try {
      ...
      if (res.success && res.user) {
        ...
        const defAddr = res.user.addresses?.find((a) => a.isDefault) || res.user.addresses?.[0];
        setShippingInfo({
          name: res.user.name || '',
      ...
      setCheckoutError(err.message || 'Google authentication failed.');
    } finally {
          ...
          {checkoutStep === 'checkout' && (
            <form id="checkout-form" onSubmit={handlePlaceOrder} className="flex flex-col gap-4">
              ...
              <div className="p-3 bg-white rounded-xl border border-[#debfc2]/30">
                ...
                {/* Guest 1-Click Google Dual-Auth Button */}
                {!currentUser && (
                  <div className="mb-3 p-2.5 bg-[#fcf9f8] border border-[#debfc2] rounded-xl text-center">
                    <button
                      type="button"
                      id="btn-checkout-google-auth"
                      onClick={handleCheckoutGoogleAuth}
                      disabled={isGoogleAuthing}
                      className="w-full flex items-center justify-center gap-2.5 bg-white hover:bg-[#fed9e2]/20 text-[#1c1b1b] border border-[#debfc2] hover:border-[#6d0026] py-2 px-3 rounded-lg text-xs font-bold transition-all shadow-2xs cursor-pointer disabled:opacity-60"
                    >
                      {isGoogleAuthing ? (
                        <div className="w-4 h-4 border-2 border-[#6d0026] border-t-transparent rounded-full animate-spin" />
                      ) : (
                      ...
                      <span>
                        {isGoogleAuthing
                          ...
                          : 'Continue with Google (1-Click Address Auto-Fill)'}
                      </span>
                    </button>
                    <p className="text-[10px] text-[#8a7174] mt-1.5">
                      Syncs your saved address directly from Google profile
                    </p>
                  </div>
                )}
                ...
                {currentUser?.addresses && currentUser.addresses.length > 1 && (
                  <div className="mb-2.5 flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] text-[#8a7174] font-medium">Select Saved:</span>
                    {currentUser.addresses.map((addr) => (
                      <button
                        ...
                        className="text-[10px] px-2 py-0.5 rounded-full border border-[#debfc2] bg-white hover:bg-[#fed9e2]/40 text-[#574144] hover:text-[#6d0026] font-medium transition-colors cursor-pointer"
                      >
                        📍 {addr.tag || addr.city}
                      </button>
                    ))}
